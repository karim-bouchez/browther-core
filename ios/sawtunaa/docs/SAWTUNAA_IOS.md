...
| Drop systématique au init_segment + early-exit epoch | (en cours) | Le hash 24-bytes était insuffisant : pub et vidéo principale partagent le même codec → pas de mismatch, cache pollué. Pragmatique : drop le cache à chaque init_segment non-initial (cost = 500ms-1s pour seek dans même vidéo, acceptable). Plus l'early-exit epoch dans preprocess Swift : on n'attend plus 5s de NSNet2 sur des chunks déjà invalidés |
