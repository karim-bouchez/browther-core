 ...
 *
 * <p>Ressuscité 2026-08-04 (ré-exposition NSFW opt-in, parité 4 plateformes)
 * depuis la version supprimée en M3 ({@code feed1ff115c^}), sans l'interface
 * {@code detectors/NudeDetector} (package retiré avec la cascade) et avec le
 * seuil en paramètre (pref {@code brave.basarunaa.nudenet_conf}, parité iOS
 * {@code NudeNetDetector.detect(threshold:)}).
 *
 ...
 */
@NullMarked
public final class NudeNetDetector {
     ...
     *
     * @param confThreshold seuil de confiance par détection (pref
     *        {@code nudenet_conf}, défaut 0.50 — parité desktop/iOS)
     */
    public Result check(Bitmap src, float confThreshold) throws OrtException {
        ...
        try (OnnxTensor input = OnnxTensor.createTensor(
            ...
            return postprocess(flat, numDetections, confThreshold);
        }
    }

    private static Result postprocess(
            FloatBuffer flat, int numDetections, float confThreshold) {
